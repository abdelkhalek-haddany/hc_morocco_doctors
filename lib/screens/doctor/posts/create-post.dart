...
class _CreatePostScreenState extends State<CreatePostScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          title: Text('Add Your Post', style: TextStyle(color: Color(0xff191A1C)),),
        ),
  ...
  validate() async {
      if (_mediaFiles.isEmpty) {
        ...
        PostModel post = PostModel(id: uuid.v1(),
            title: titleController.text,
            thumbnail: image_url,
            department: departmentID,
            status: "accepted",
            content: contentController.text,
            doctor_id: _id.toString());
        await FireStoreUtils.createPost(post);
